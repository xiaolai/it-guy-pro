...
2. Dispatch the `mac-it-guy-pro:toolsmith` agent (via Task) with the upgrade spec.
...
- A tool's real run exits non-zero → show its error message verbatim, do not retry with `--go`, and offer `/mac-it-guy-pro:fix` framing: diagnose why before running again.
