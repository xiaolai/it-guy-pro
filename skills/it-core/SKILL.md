...
2. **Trash, never rm.** Every deletion goes through the Finder Trash (recipe in the `macos-recipes` skill) so the user can undo it. A PreToolUse hook enforces this — if it blocks you, use the Trash recipe instead of rephrasing the rm.
   ...
   **The hook is global, and that is a limitation rather than a design goal.** A PreToolUse hook sees every Bash command in the session and the payload names no originating plugin, so it cannot restrict itself to this plugin's own workflows. `ITGUY_GUARD` therefore lets the user choose how much applies: `data` (the default — file protection only, no machine policy, no prompts), `strict` (all of it), `relaxed` (all denials, no prompts), `off`. A typo falls back to `strict`, which is stricter than the default on purpose: an unset variable means no choice was made, a malformed one means a choice failed. If a user complains the guard is interfering with unrelated work, they are almost certainly at `strict` — `data` is the answer, not disabling the plugin.

   The hook has two tiers and they are not equivalent. **Deny** covers user content, the Trash, backups, disk erasure, and destruction of whole system or user trees; no level except `off` weakens it. **Ask** covers recursive deletes of unrecognised paths and similar unrecoverable-but-often-legitimate work, and is **off by default** — a prompt written in shell asks a question this plugin's reader cannot evaluate, and constant prompts train anyone to approve without reading. `strict` turns it on for users who can read it. Recognised build output (`node_modules`, `target`, `build`, `.venv` and similar) never prompts even then.

   **Every block states the level it is running at and how to change it.** Never tell a user to disable the plugin to get past a block; read the level out of the message and suggest the narrower setting.
