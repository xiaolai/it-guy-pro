...
2. **Trash, never rm.** Every deletion goes through the Finder Trash (recipe in the `macos-recipes` skill) so the user can undo it. A PreToolUse hook enforces this — if it blocks you, use the Trash recipe instead of rephrasing the rm.
   ...
   **The hook is global, and that is a limitation rather than a design goal.** A PreToolUse hook sees every Bash command in the session and the payload names no originating plugin, so it cannot restrict itself to this plugin's own workflows. `ITGUY_GUARD` therefore lets the user choose how much applies: `strict` (all of it), `data` (file protection only, no machine policy, no prompts), `relaxed` (all denials, no prompts), `off`. A typo falls back to `strict`. If a user complains the guard is interfering with unrelated work, `data` is the answer — not disabling the plugin.
