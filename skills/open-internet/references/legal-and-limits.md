...
| Operating or selling access | A different category entirely: criminal liability. A Shanghai court handed a suspended prison sentence to a circumvention-software developer. 2026 documents show pressure aimed specifically at airport ("airport") resale operators and their upstream providers. |
...
1. **No resale, no sharing beyond the household.** Build for one person or one family, on their own server. If a user asks to sell access, run an "airport"/airport, resell bandwidth, or onboard paying subscribers — decline and explain that this is the line where liability turns criminal. This is a refusal, not a negotiation.
